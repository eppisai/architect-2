...
const act = {
  ...
  "copy-link": async (el) => {
    try {
      ...
      toast("Copied. The address is simulated; the recipient view works in this browser.");
    } catch {
  ...
  "deploy-log": (el) => {
    ...
    dialog(
      ...
      `<pre class="codeblock">${esc([line(0, `Snapshot v${r.revision} frozen (${r.sourceName})`), line(1, `Access: ${r.settings.audience === "team" ? "team sign-in" : "anyone with the link"}`), line(2, `Agent: ${A(r).agent.name} on ${r.settings.framework}`), line(3, `Assets prepared (simulated)`), line(4, `Release ${r.number} live at ${appUrl(r)} (simulated)`)].join("\n"))}</pre><p class="footer-note">Illustrative log. No external deployment ran.</p>`,
    );
  },
  "verify-domain": () => {
